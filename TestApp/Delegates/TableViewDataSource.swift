    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .firstSection:
            ...
            case 0:
                ...
                cell = tableView.dequeueReusebleCell(with: cellModel, for: indexPath)
            default:
                if let daily = viewModel.weather.daily?.first {
                    let data = Converter.convert(daily)
                    let cellModel = TodayCellModel(weekDay: data.weekDay, tempMin: data.tempMin, tempMax: data.tempMax)
                    cell = tableView.dequeueReusebleCell(with: cellModel, for: indexPath)
                }
            }
        case .secondSection:
            let dailyCount = viewModel.weather.daily?.count ?? 0
            let daysCount = dailyCount - 1
            let isDailyRow = indexPath.row < daysCount
            
            switch isDailyRow {
            case true:
                if let daily = viewModel.weather.daily?[indexPath.row+1] {
                    let data = Converter.convert(daily)
                    let cellModel = DailyCellModel(weekDay: data.weekDay, icon: data.icon, tempMin: data.tempMin, tempMax: data.tempMax)
                    cell = tableView.dequeueReusebleCell(with: cellModel, for: indexPath)
            ...
            default:
                ...
                if indexPath.row == startIndex {
                    if let today = viewModel.weather.daily?.first {
                        ...
                        let text = "Today: \(currentWeather.description), The heighest temperature is \(todayData.tempMax)°C. The lowest temperature is \(todayData.tempMin)°C."
